...
Cube::Cube(Mesh* mesh_, Shader* shader_) : mesh(mesh_), shader(shader_)
{
	pos.x = 0;
	pos.y = 0;
	pos.z = 0;
...
Cube::Cube(Vec3 pos_, Vec3 vel_, Vec3 accel_, float mass_, float maxSpeed_, Mesh* mesh_, Shader* shader_, CollisionBox* collisionBox_) : mesh(mesh_), shader(shader_), collisionBox(collisionBox_)
{
	pos.x = pos_.x;
	pos.y = pos_.y;
	pos.z = pos_.z;
...
void Cube::ApplyForce(Vec3 force)
{
